...
import gsap from "gsap"; // Đảm bảo bạn đã cài gsap
...
const animateCat = () => {
  ...
  // Lặp lại note trong SVG nếu cần
  notes.forEach((note) => {
  ...
  // Hoạt hình cho các notes
  const animateNotes = (els) => {
    els.forEach((el) => {
      gsap.set(el, {
        stroke: gsap.utils.random([green, pink, blue, orange, cyan], true),
    ...
    return gsap.fromTo(
      ...
      },
      {
        ...
        rotation: `+=${gsap.utils.random(20, 30, 1)}`,
        x: `+=${gsap.utils.random(40, 60, 1)}`,
...
const CatComponent = () => {
  useEffect(() => {
    console.log("CatComponent mounted");
    const musicNotes = document.querySelectorAll(".note");
    console.log(musicNotes); // Kiểm tra xem có phần tử .note không
    animateCat(); // Khởi động hoạt hình khi component được render
  }, []);
  return (
    <div classname="CatFrame">
      <div className="containerCat">
        <svg
          ...
          style={{ transform: "scale(0.8)" }} //giảm tỉ lệ
        >
          {"{"}" "{"}"}
          <g id="bongo-cat">
            <g className="head">
